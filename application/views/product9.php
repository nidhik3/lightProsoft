<link rel="stylesheet" href="assets/css/n1.css">
<section id="services" class="services  lightblue  pt-120 pb-60">
    <div class="container">
        <div class="row text-center pb-4">
            <h3 class="tm-gold-text tm-form-title"><span class="gradient-text">MOBILE SOLUTIONS</span></h3>
        </div>

        <div class="row">
            <div class="col-md-6">
                <p> PROSOFT provides the different kind of Mobile application and solutions which helps customer to perform.</p>
                <ul class="tick pt-2">
                    <li>Book Appointments</li>
                    <li>Payments Options</li>
                    <li>Patient Registrations</li>
                    <li>Print Invoice / Receipt</li>
                    <li>Print Lab Reports</li>
                    <li>Print Radiology Reports</li>
                    <li>Check Patient Discharge Summary</li>
                    <li>Check Patient EMR</li>
                    <li>Monitor Vitals</li>
                </ul>

            </div>
            <div class="col-md-6">
                <img src="assets/image/ris.jpg" alt="" width="100%" style="border-radius: 20px;">
            </div>
        </div>

    </div>
    <br />

</section>



<script>
    $(document).ready(function() {
        $('a[href$="#"]').addClass(" active");
    })
</script>